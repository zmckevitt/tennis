...
struct vec diff(struct vec v1, struct vec v2) {
    struct vec tmp;
...
int draw(struct vec pos, int last1, int last2) {

    // make these dependent on height/width or something
    struct vec guy1 = init_vec(16,4);
    struct vec guy2 = init_vec(WIDTH - 16 - 1, HEIGHT - 4 -1);
    ...
    for(int y=0; y<HEIGHT; ++y) {
        for(int x=0; x<WIDTH; ++x) {
            if(x == 0 || y == 0 || x == WIDTH - 1 || y == HEIGHT -1) {
                ...
                if(pos.x < scalex || pos.x > WIDTH-scalex-1) {
                    row[x] = 'X';
                    quit = 1;
                }
                else {
...
int main(int argc, char** argv) {
    ...
    struct vec guy1 = init_vec(15,4);
    struct vec guy2 = init_vec(WIDTH - 15 - 1, HEIGHT - 4 -1);

    // give guy1 the ball first
    struct vec pos = guy1;

    // choose random initial velocity
    struct vec vel = init_vec(1,1);
